...
<script lang="ts">
	...
	let { friendName }: Props = $props();

	// Input state
	let input = $state('');
	let isConnected = $derived(dmSocketState.current === 'open');
	...
	function handleSubmit() {
		...
		if (pendingFile && text) {
			sendDmMessageWithFile(text, pendingFile);
			input = '';
			pendingFile = null;
		} else if (pendingFile) {
			handleSendFile();
